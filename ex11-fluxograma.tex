...
\begin{tikzpicture}[node distance=4cm]
	...
	\tikzstyle{print} = [rectangle, minimum width=3cm, minimum height=1cm, text centered, draw=black, fill=blue!10]
	\tikzstyle{input} = [trapezium, trapezium left angle=70, trapezium right angle=110, minimum width=3cm, minimum height=1cm, text centered, draw=black, fill=orange!20]
	\tikzstyle{decision} = [diamond, minimum width=3cm, minimum height=1cm, text centered, draw=black, fill=yellow!20]
	\tikzstyle{line} = [draw, -latex']

	% Colocar nodos com espaçamento ajustado
	\node (inicio) [startend] {Início};
	\node (txta) [print, below of=inicio, node distance=2.5cm] {Apresentação do reino e do monstro};
	\node (inp1) [input, below of=txta, node distance=2.5cm] {Escolha da arma (espada ou arco)};
	\node (tot1) [decision, below of=inp1, node distance=4.5cm] {Escolha = espada?};
	\node (maior) [decision, below of=tot1, node distance=5.5cm] {Ação: atacar ou fugir?};
	\node (sim) [print, right of=tot1, node distance=6.5cm] {Morreu (escolheu arco)};
	\node (nao) [print, right of=maior, node distance=6.5cm] {Morreu (fugiu)};
	\node (fim) [startend, below of=maior, node distance=4.5cm] {Venceu (atacou)};
   \node (fim1) [startend, below of=fim, node distance=2.5cm] {Fim};

    % Desenhar as setas
	\path [line] (inicio) -- (txta);
	\path [line] (txta) -- (inp1);
	\path [line] (inp1) -- (tot1);
	\path [line] (tot1) -- node[anchor=south] {Sim} (maior);
	\path [line] (tot1) -- node[anchor=north] {Não} (sim);
	\path [line] (maior) -- node[anchor=east] {Atacar} (fim);
	\path [line] (maior) -- node[anchor=north] {Fugir} (nao);
    \path [line] (fim) -- (fim1);
...
\begin{enumerate}
    ...
    \end{description}
      ...
      \begin{description}
        \item [Email:] \email{jvsms@poli.br}
        \item [Endereço:]
        \item [Matrícula:]
        \item [CPF:]
        \item [RG:]
